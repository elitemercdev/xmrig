...
#include <string.h>      /* get the memmove/memset functions */
...
static int Skein_512_Init(Skein_512_Ctxt_t *ctx, size_t hashBitLen)
    ...
    switch (hashBitLen)
        ...
        case  256: memmove(ctx->X,SKEIN_512_IV_256,sizeof(ctx->X));  break;
#endif
        default:
...
static int Skein_512_Update(Skein_512_Ctxt_t *ctx, const u08b_t *msg, size_t msgByteCnt)
    ...
    if (msgByteCnt + ctx->h.bCnt > SKEIN_512_BLOCK_BYTES)
        ...
        if (ctx->h.bCnt)                              /* finish up any buffered message data */
            ...
            if (n)
                ...
                memmove(&ctx->b[ctx->h.bCnt],msg,n);
                msgByteCnt  -= n;
                msg         += n;
                ctx->h.bCnt += n;
                }
    ...
    if (msgByteCnt)
        ...
        memmove(&ctx->b[ctx->h.bCnt],msg,msgByteCnt);
...
static int Skein_512_Final(Skein_512_Ctxt_t *ctx, u08b_t *hashVal)
    ...
    memmove(X,ctx->X,sizeof(X));       /* keep a local copy of counter mode "key" */
    for (i=0;i*SKEIN_512_BLOCK_BYTES < byteCnt;i++)
        ...
        memmove(ctx->X,X,sizeof(X));   /* restore the counter mode key for next time */
        }
    return SKEIN_SUCCESS;
    }
...
void xmr_skein(const SkeinBitSequence *data, SkeinBitSequence *hashval){
  ...
  memmove(state.u.ctx_512.X,SKEIN_512_IV_256,sizeof(state.u.ctx_512.X));
